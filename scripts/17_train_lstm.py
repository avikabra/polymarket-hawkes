"""Script 17: Train LSTM predictor.
...
Reads:  data/analysis/shock_embeddings.parquet
        ...
        models/hparam_search/lstm_{category}_results.json  (optional)
Writes: models/checkpoints/lstm_{category}_{embedding}_best.pt
...
def _load_best_hparams(category: str) -> dict | None:
    results_path = HPARAM_DIR / f"lstm_{category}_results.json"
...
def main() -> None:
    ...
    proj_dim = int(hparams.get("proj_dim", arch_cfg.get("proj_dim", 256)))
    dropout = float(hparams.get("dropout", arch_cfg.get("dropout", 0.1)))
    use_gru = bool(arch_cfg.get("use_gru", False))
    ...
    # Load best weights and run test inference
