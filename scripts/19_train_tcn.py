...
Reads:  data/analysis/shock_embeddings.parquet
        ...
        models/hparam_search/tcn_{category}_results.json  (optional)
Writes: models/checkpoints/tcn_{category}_{embedding}_best.pt
...
def _load_best_hparams(category: str) -> dict | None:
    results_path = HPARAM_DIR / f"tcn_{category}_results.json"
...
def main() -> None:
    ...
    if hparams is None:
        log.info("tcn_train", msg="No hparam search results found; using defaults.")
        hparams = {"K": 10, "channels": 256, "num_blocks": 3, "dropout": 0.1}
    else:
        log.info("tcn_train", msg=f"Loaded hparams from search: {hparams}")

    K = int(hparams.get("K", arch_cfg.get("window_size", 10)))
    channels = int(hparams.get("channels", arch_cfg.get("channels", 256)))
    num_blocks = int(hparams.get("num_blocks", arch_cfg.get("num_blocks", 3)))
    dropout = float(hparams.get("dropout", arch_cfg.get("dropout", 0.1)))

    batch_size = int(arch_cfg.get("batch_size", 64))
    max_epochs = int(arch_cfg.get("max_epochs", 80))
    patience = int(arch_cfg.get("patience", 12))
    ...
    if args.resume and ckpt_path.exists():
        ...
        log.info("tcn_train", msg=f"Resumed from {ckpt_path}")
